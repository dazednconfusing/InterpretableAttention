...
def calc_metrics_classification(target, predictions):
    ...
    if len(np.unique(target)) < 4:
        ...
        rep = json_normalize(
            classification_report(target, predict_classes, output_dict=True), sep="/"
        )
        # rep['idx'] = 0
        # rep = pd.DataFrame(rep, index=[1])

    else:
        rep = {}
    rep["accuracy"] = accuracy_score(target, predict_classes)
    # rep.update({"accuracy": accuracy_score(target, predict_classes)})
    if predictions.shape[-1] == 2:
        rep["roc_auc"] = roc_auc_score(target, predictions[:, 1])
        rep["pr_auc"] = average_precision_score(target, predictions[:, 1])
...
def calc_metrics_multilabel(target, predictions):
    ...
    for i in range(nlabels):
        ...
        rep_i = json_normalize(
            classification_report(
                target[:, i], predict_classes[:, i], output_dict=True
            ),
            sep="/",
        )
        rep_i["accuracy"] = accuracy_score(target[:, i], predict_classes[:, i])
        rep_i["roc_auc"] = roc_auc_score(target[:, i], predictions[:, i])
        rep_i["pr_auc"] = average_precision_score(target[:, i], predictions[:, i])
        # rep_i.update({"accuracy": accuracy_score(target[:, i], predict_classes[:, i])})
        # rep_i.update({"roc_auc": roc_auc_score(target[:, i], predictions[:, i])})
        # rep_i.update(
        #     {"pr_auc": average_precision_score(target[:, i], predictions[:, i])}
        # )
