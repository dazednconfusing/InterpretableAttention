...
class TanhAttention(Attention):
    ...
    def randomize_weights(self):
        """All weights are redrawn from a
        gaussian with the same mean and std dev as the non-randomized distribution.

        Author: Sebastian Peralta
        """
        with torch.no_grad():
            a1_mean = self.attn1.weight.mean().item()
            a1_var = torch.var(self.attn1.weight)

            a2_mean = self.attn2.weight.mean().item()
            a2_var = torch.var(self.attn2.weight)

            new_attn1 = torch.randn(self.attn1.weight.size())
            new_attn2 = torch.randn(self.attn2.weight.size())

            new_attn1 = a1_mean + torch.sqrt(a1_var).item() * new_attn1
            new_attn2 = a2_mean + torch.sqrt(a2_var).item() * new_attn2

            self.attn1.weight = torch.nn.Parameter(new_attn1)
            self.attn2.weight = torch.nn.Parameter(new_attn2)
    ...
    def forward(self, data):
        ...
        if isTrue(data, "erase_random"):
            rand_len = (
                torch.rand(size=lengths.size()).to(device) * (lengths).float()
            ).long()
...
class MultiTanhAttention(Attention):
    def __init__(self, hidden_size, heads=2):
        ...
        self.attn1 = nn.ModuleList(
            [nn.Linear(hidden_size, hidden_size // 2).to(device) for _ in range(heads)]
        )
        self.attn2 = nn.ModuleList(
            [
                nn.Linear(hidden_size // 2, 1, bias=False).to(device)
    ...
    def forward(self, data):
        ...
        proj_hiddens = torch.unbind(
            self.projection(hidden).view(b, l, self.heads, h), dim=2
        )  # heads - [ (B,L,H), ...]
