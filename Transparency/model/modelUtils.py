...
def get_sorting_index_with_noise_from_lengths(lengths, noise_frac):
    if noise_frac > 0:
        noisy_lengths = [
            x + np.random.randint(np.floor(-x * noise_frac), np.ceil(x * noise_frac))
            for x in lengths
        ]
    else:
...
class BatchHolder:
    def __init__(self, data, is_embds=False):
        ...
        if is_embds:
            # if LongTensor is used, embeddings will be truncated to 0
    ...
    def repeat(self, *sizes):
        self.lengths = self.lengths.repeat(sizes)
        self.seq = self.seq.repeat(sizes)
        self.masks = self.masks.repeat(sizes)

    def generate_permutation(self):
        """Generates random permutation of attention weight inputs

        Returns:
            np.array: Mapped permutation indexes
    ...
    def generate_uniform_attn(self):
        ...
        inv_l = 1.0 / self.lengths.cpu().data.numpy()
        attn += inv_l[:, None]
