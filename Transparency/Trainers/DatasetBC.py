...
class DataHolder:
    ...
    def filter(self, idxs):
        data_kwargs = {
            key: [getattr(self, key)[i] for i in idxs] for key in self.attributes
        }
        return DataHolder(**data_kwargs)
...
class Dataset:
    def __init__(self, name, path, min_length=None, max_length=None, args=None):
        ...
        X, Xd, Xt = (
            self.vec.seq_text["train"],
            self.vec.seq_text["dev"],
            self.vec.seq_text["test"],
        )
        y, yd, yt = (
            self.vec.label["train"],
            self.vec.label["dev"],
            self.vec.label["test"],
        )
    ...
    def display_stats(self):
        ...
        json.dump(
            stats, open("graph_outputs/" + outdir + "/" + self.name + ".txt", "w")
        )
        print(stats)
...
def SST_dataset(args=None):
    dataset = Dataset(
        name="sst", path="preprocess/SST/vec_sst.p", min_length=5, args=args
    )
...
def IMDB_dataset(args=None):
    dataset = Dataset(
        name="imdb", path="preprocess/IMDB/vec_imdb.p", min_length=6, args=args
    )
    set_balanced_pos_weight(dataset)
    return dataset
...
def News20_dataset(args=None):
    dataset = Dataset(
        name="20News_sports",
        path="preprocess/20News/vec_20news_sports.p",
        min_length=6,
        max_length=500,
        args=args,
    )
...
def Yelp(args=None):
    dataset = Dataset(
        name="Yelp",
        path="preprocess/Yelp/vec_yelp.p",
        min_length=6,
        max_length=150,
        args=args,
    )
...
def Amazon(args=None):
    dataset = Dataset(
        name="Amazon",
        path="preprocess/Amazon/vec_amazon.p",
        min_length=6,
        max_length=100,
        args=args,
    )
...
def ADR_dataset(args=None):
    dataset = Dataset(
        name="tweet",
        path="preprocess/Tweets/vec_adr.p",
        min_length=5,
        max_length=100,
        args=args,
    )
...
def Anemia_dataset(args=None):
    dataset = Dataset(
        name="anemia", path="preprocess/MIMIC/vec_anemia.p", max_length=4000, args=args
    )
    set_balanced_pos_weight(dataset)
    return dataset
...
def Diabetes_dataset(args=None):
    dataset = Dataset(
        name="diab",
        path="preprocess/MIMIC/vec_diabetes.p",
        min_length=6,
        max_length=4000,
        args=args,
    )
